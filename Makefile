...
compile: clean
	clear
	icpx -std=c++20 -fsycl -Wno-unqualified-std-cast-call -I$(include_folder) $(analyzer_folder)/*.cpp $(ENVI_folder)/*.cpp -o $(binary)

compile_test:
	clear
	icpx -std=c++20 -fsycl -Wno-unqualified-std-cast-call -I$(include_folder) $(test_folder)/*.cpp $(ENVI_folder)/*.cpp $(analyzer_folder)/Analyzer_tools.cpp -o $(test_binary)
